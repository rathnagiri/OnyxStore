...
public class WhenLogin
{
    ...
    public async Task GivenSuccess_Call_Then_ReturnOk()
    {
        ...
        _mockAuthService.Setup(s => s.LoginAsync(_dto)).ReturnsAsync(new AuthResult
        {
            Status = AuthStatusResult.Success,
            Token = token
        });

        var result = await _controller.Login(_dto);
    ...
    public async Task Given_BadRequest_WhenEmailExists_Then_ReturnBadRequest()
    {
        _mockAuthService.Setup(s => s.LoginAsync(_dto)).ReturnsAsync(new AuthResult
        {
            Status = AuthStatusResult.InvalidEmailOrPassword
        });
        ...
        badRequest!.Value.ToString().ShouldContain(AuthStatusResult.InvalidEmailOrPassword.ToString());
    }
}